...
import { trackInGameQuizAttempt, runningGameAnalytics } from '../stubs/analytics'
...
export class QuizManager {
  ...
  private async retryLoadKeywordsForQuiz(): Promise<void> {
    // 랜딩페이지 포팅: 키워드 API 없음 — registry 재확인 후 없으면 정답 처리
    const kw = this.scene.game.registry.get("keywords") as KeywordEntry[] | undefined;
    if (kw && kw.length >= 3) {
      this.keywords = kw;
      this.showQuizAnswer();
      return;
